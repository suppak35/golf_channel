...
/* Version 1.3 : 15/05/2025 - Suppachai K. - add case TARIFF_RATE_TYPE                                                 */
/*-----------------------------------------------------------------------------------------------------------------------------*/
...
DECLARE
  ...
  CURSOR MAIN_CUR IS
     ...
     CASE -- add max 15/05/2025 v.1.3
         WHEN TARIFF_RATE_TYPE = 'PER' THEN
          ABS(SUM(CASE
                    WHEN REPORT_TYPE = 'ADJUST' THEN
         ...
         ELSE
          ABS(SUM(CASE
                    WHEN REPORT_TYPE = 'ADJUST' THEN
                     (TARIFF_RATE) * TOT_TRANS * (-1)
                    ELSE
                     (TARIFF_RATE) * TOT_TRANS
                  END))
       END TOT_EE_AMT,
           TARIFF_RATE,
           STATUS,
      ...
      FROM (SELECT P_REPORT_REF REPORT_REFERENCE,
                  ...
                  -- R.TARIFF_ID,
                   ...
                   (SELECT TARIFF_RATE_TYPE FROM SS_TARIFF_DETAIL WHERE TARIFF_ID = VT.TARIFF_ID)  TARIFF_RATE_TYPE,
                   B.STATUS,
                   'CSH' SHARE_BASIS
             ...
             GROUP BY R.INVOICING_CO_ID,
                  ...
                  -- R.TARIFF_ID,
                   ...
                   REPORT_CODE, DATA_TYPE,TARIFF_ID, REPORT_TYPE, EVENT_MONTH, USAGE_RATE, VAS_NO, TARIFF_RATE,
                    ...
                    EE_ID, REPORT_CODE, DATA_TYPE, TARIFF_RATE,TARIFF_RATE_TYPE));
